...
重点要求：
1. 内容必须与频道「${channelName}」的定位（${channelDescription}）密切相关，但每次都要寻找**全新**的切入角度或细分话题。
2. 绝对不要重复最近已发布的话题或相似的角度（仔细查看历史内容）。每次生成的内容都要有明显差异（可以是分享技巧、分析趋势、提出疑问、推荐资源、或者个人感悟等）。
3. 选一个最有价值、最能引发读者共鸣的独特视角。
4. 设计抓眼球的开头句子（不要用"随着...""在当今..."等老套套路）
5. 指定写作语气和结构形式，贴近真人写作，每次必须尝试与前几篇不同的排版结构，彻底避免 AI 腔调和死板的段落布局。
